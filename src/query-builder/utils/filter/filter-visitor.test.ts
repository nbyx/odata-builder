...
// ODataDateStringLiteral, // Nicht direkt benötigt für Tests, da string ausreicht, wenn Typinferenz funktioniert
...
describe('visitLambdaFilter', () => {
    ...
    it('should handle a lambda filter with a field in the expression on object array', () => {
        ...
        const result = visitor.visitLambdaFilter(filter);
        expect(result).toBe("complexTags/any(s: s/tagValue eq 'important')");
    });
    ...
    it('should throw an error for an invalid lambda filter structure (missing expression)', () => {
        ...
        expect(() => visitor.visitLambdaFilter(invalidLambdaObject)).toThrow(
...
describe('visitCombinedFilter', () => {
    ...
    it('should throw an error for an invalid sub-filter in combined filter', () => {
        ...
        expect(() => visitor.visitCombinedFilter(filterWithInvalidSubFilter)).toThrow(
            /Unsupported expression type in visitExpression/,
        );
    });
    ...
    it('should throw if combined filter has no "filters" array', () => {
        ...
        expect(() => visitor.visitCombinedFilter(filterMissingFiltersArray)).toThrow(
     ...
     it('should throw if sub-filter in combined filter is null', () => {
        ...
        expect(() => visitor.visitCombinedFilter(filterWithNullSubFilter)).toThrow(
...
describe('visitBasicFilter with functions', () => {
    ...
    it('should handle string concat function with literal and fieldReference', () => {
        ...
        const result = visitor.visitBasicFilter(filter);
        expect(result).toBe("concat(details/name, '-PK', details/code) eq 'ProductName-PK123'");
    });

    it('should handle string contains (as function) with fieldReference, direct boolean', () => {
        const filter: QueryFilter<ItemType> = {
            field: 'stringField',
            function: { type: 'contains', value: { fieldReference: 'anotherStringField' } },
        };
        const result = visitor.visitBasicFilter(filter);
    ...
    it('should handle string substring function (start only, returns string)', () => {
     ...
     it('should handle string substring function (start and length, returns string)', () => {
        const filter: QueryFilter<ItemType> = {
            field: 'stringField',
            function: { type: 'substring', start: 1, length: { fieldReference: 'numericField' } },
            operator: 'eq',
            value: 'sub',
        };
        const result = visitor.visitBasicFilter(filter);
        expect(result).toBe("substring(stringField, 1, numericField) eq 'sub'");
    });

    it('should handle tolower, toupper, trim functions (return string)', () => {
        const filterLower: QueryFilter<ItemType> = { field: 'stringField', function: { type: 'tolower' }, operator: 'eq', value: 'lower case version' };
        expect(visitor.visitBasicFilter(filterLower)).toBe("tolower(stringField) eq 'lower case version'");

        const filterUpper: QueryFilter<ItemType> = { field: 'stringField', function: { type: 'toupper' }, operator: 'eq', value: 'UPPER CASE VERSION' };
        expect(visitor.visitBasicFilter(filterUpper)).toBe("toupper(stringField) eq 'UPPER CASE VERSION'");

        const filterTrim: QueryFilter<ItemType> = { field: 'stringField', function: { type: 'trim' }, operator: 'eq', value: 'trimmed version' };
        expect(visitor.visitBasicFilter(filterTrim)).toBe("trim(stringField) eq 'trimmed version'");
    });


    // Arithmetic Functions
    it('should handle arithmetic functions: add, sub, mul, div, mod (return number)', () => {
        const filterAdd: QueryFilter<ItemType> = { field: 'age', function: {type: 'add', operand: 5}, operator: 'eq', value: 15};
        expect(visitor.visitBasicFilter(filterAdd)).toBe("age add 5 eq 15");
        const filterSub: QueryFilter<ItemType> = { field: 'age', function: { type: 'sub', operand: 5 }, operator: 'eq', value: 10 };
        expect(visitor.visitBasicFilter(filterSub)).toBe("age sub 5 eq 10");
        const filterMul: QueryFilter<ItemType> = { field: 'age', function: { type: 'mul', operand: 2 }, operator: 'eq', value: 40 };
        expect(visitor.visitBasicFilter(filterMul)).toBe("age mul 2 eq 40");
        const filterDiv: QueryFilter<ItemType> = { field: 'age', function: { type: 'div', operand: { fieldReference: 'valueA' } }, operator: 'eq', value: 2 };
        expect(visitor.visitBasicFilter(filterDiv)).toBe("age div valueA eq 2");
        const filterMod: QueryFilter<ItemType> = { field: 'age', function: { type: 'mod', operand: 5 }, operator: 'eq', value: 0 };
    ...
    it('should handle numeric functions: round, floor, ceiling (return number)', () => {
        const filterRound: QueryFilter<ItemType> = { field: 'details/value', function: { type: 'round' }, operator: 'eq', value: 10 };
        expect(visitor.visitBasicFilter(filterRound)).toBe("round(details/value) eq 10");
        const filterFloor: QueryFilter<ItemType> = { field: 'details/value', function: { type: 'floor' }, operator: 'eq', value: 9 };
        expect(visitor.visitBasicFilter(filterFloor)).toBe("floor(details/value) eq 9");
        const filterCeiling: QueryFilter<ItemType> = { field: 'details/value', function: { type: 'ceiling' }, operator: 'eq', value: 10 };
        expect(visitor.visitBasicFilter(filterCeiling)).toBe("ceiling(details/value) eq 10");
    });
    ...
    it('should handle date() function with fieldReference and OData date literal', () => {
        const filter: QueryFilter<ItemType> = {
            ...
            function: { type: 'date', field: { fieldReference: 'anotherDateField' } },
            operator: 'eq',
            value: '2023-07-15',
            removeQuotes: true,
        };
    ...
    it('should handle time() function with fieldReference and OData time literal', () => {
        const filter: QueryFilter<ItemType> = {
            field: 'anotherDateField',
            function: { type: 'time', field: { fieldReference: 'anotherDateField' } },
            operator: 'gt',
            value: '12:00:00.000',
            removeQuotes: true,
        };
    ...
    it('should handle year, month, day, hour, minute, second functions (return number)', () => {
        const filterYear: QueryFilter<ItemType> = { field: 'startDate', function: { type: 'year' }, operator: 'eq', value: 2023 };
        expect(visitor.visitBasicFilter(filterYear)).toBe("year(startDate) eq 2023");
        const filterMonth: QueryFilter<ItemType> = { field: 'dateField', function: {type: 'month'}, operator: 'eq', value: 12};
        expect(visitor.visitBasicFilter(filterMonth)).toBe("month(dateField) eq 12");
        const filterDay: QueryFilter<ItemType> = { field: 'dateField', function: {type: 'day'}, operator: 'eq', value: 15};
        expect(visitor.visitBasicFilter(filterDay)).toBe("day(dateField) eq 15");
        const filterHour: QueryFilter<ItemType> = { field: 'dateField', function: {type: 'hour'}, operator: 'eq', value: 10};
        expect(visitor.visitBasicFilter(filterHour)).toBe("hour(dateField) eq 10");
        const filterMinute: QueryFilter<ItemType> = { field: 'dateField', function: {type: 'minute'}, operator: 'eq', value: 30};
        expect(visitor.visitBasicFilter(filterMinute)).toBe("minute(dateField) eq 30");
        const filterSecond: QueryFilter<ItemType> = { field: 'dateField', function: {type: 'second'}, operator: 'eq', value: 0};
        expect(visitor.visitBasicFilter(filterSecond)).toBe("second(dateField) eq 0");
    });
    ...
    it('should throw for concat without values', () => {
        const filter = { field: 'id', function: { type: 'concat' }, operator: 'eq', value: 'test' } as unknown as QueryFilter<ItemType>;
        expect(() => visitor.visitBasicFilter(filter)).toThrow(/Invalid function definition for 'concat': 'values' array is missing/);
    });
    it('should throw for contains without value property in function def', () => {
        const filter = { field: 'id', function: { type: 'contains' }, operator: 'eq', value: true } as unknown as QueryFilter<ItemType>;
        expect(() => visitor.visitBasicFilter(filter)).toThrow(/Invalid function definition for 'contains': 'value' property is missing/);
    });
    it('should throw for substring without start property in function def', () => {
        const filter = { field: 'id', function: { type: 'substring' }, operator: 'eq', value: 't' } as unknown as QueryFilter<ItemType>;
        expect(() => visitor.visitBasicFilter(filter)).toThrow(/Invalid function definition for 'substring': 'start' property is missing/);
    });
    it('should throw for add without operand property in function def', () => {
        const filter = { field: 'age', function: { type: 'add' }, operator: 'eq', value: 10 } as unknown as QueryFilter<ItemType>;
        expect(() => visitor.visitBasicFilter(filter)).toThrow(/Invalid function definition for 'add': 'operand' property is missing/);
    });
    it('should throw for date function without field property in function def', () => {
        const filter = { field: 'startDate', function: { type: 'date' }, operator: 'eq', value: '2023-01-01', removeQuotes: true } as unknown as QueryFilter<ItemType>;
        expect(() => visitor.visitBasicFilter(filter)).toThrow(/Invalid function definition for 'date': 'field' property is missing/);
    });
    it('should throw for date function with invalid fieldReference in function def', () => {
        const filter = { field: 'startDate', function: { type: 'date', field: {} }, operator: 'eq', value: '2023-01-01', removeQuotes: true } as unknown as QueryFilter<ItemType>;
        expect(() => visitor.visitBasicFilter(filter)).toThrow(/Invalid FieldReference: It must be an object with a 'fieldReference' property that is a non-empty string/);
    });
    it('should throw for function with empty type string', () => {
        const filter = { field: 'id', function: { type: ' ' }, operator: 'eq', value: 'test' } as unknown as QueryFilter<ItemType>;
         expect(() => visitor.visitBasicFilter(filter)).toThrow(/Invalid function definition on field "id": "type" property is missing, not a string, or empty./);
    });
...
describe('Functions in Lambda Expressions', () => {
    ...
    it('should handle tolower function within a lambda on object array property', () => {
        ...
        const result = visitor.visitLambdaFilter(filter);
        expect(result).toBe("complexTags/all(s: tolower(s/tagValue) eq 'important')");
    });
    ...
    it('should handle year function within a lambda on date array', () => {
        ...
        const result = visitorDates.visitLambdaFilter(filter as LambdaFilter<ItemType>);
